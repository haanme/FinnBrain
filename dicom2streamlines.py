...
if __name__ == "__main__":
    ...
    # Convrt for dtifit
    dwifile, bval_file, bvec_file = conv.nrrd2nii(qcfile, args.subject)
    no_bvalues = np.genfromtxt(bval_file)
    bval_file = create_bvals(no_bvalues, args.subject, '_motioncorrected_QCed_fixed')

    # dwifile, bvalfile, bvecfile = remove_vols(dwifile, bvalfile, bvecfile, [45, 48, 49, 50], args.subject)
    mask_file = runbet(dwifile, 0.5, args.subject)
    fsl.dtifit(dwifile, mask_file, bval_file, bvec_file, args.subject)

    # Convert QCced to BEDPOSX
    copy_to_results(dwifile, 'data.nii.gz', args.subject)
    copy_to_results(bvalfile, 'bvals', args.subject)
    copy_to_results(bvecfile, 'bvecs', args.subject)
    copy_to_results(mask_file, 'nodif_brain_mask.nii.gz', args.subject)

if 0:
    brainmask_file = runbet(dwifile, 0.5, args.subject)
    dilated_brainmask_file = fsl.fslmath_dilate(brainmask_file, 'mean', 10, args.subject)
    backgroundmask_file = fsl.fslmath_op_custom(dilated_brainmask_file, '-binv', args.subject)

    SNR = calculateSNR(dwifile, brainmask_file, backgroundmask_file)
    for SNR_i in range(len(SNR)):
        print "vol" + str(SNR_i) + " SNR:" + str(SNR[SNR_i])
