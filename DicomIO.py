...
class DicomIO:
    ...
    def ReadDICOM_slices(self, path, verbose=0):
        ...
        for filename_i in range(len(filenames)): 
            ...
            if verbose > 0 and (np.mod(filename_i, print_step) == 0 or filename_i == (len(filenames)-1)):
    ...
    def ReadDICOM_frames(self, path, verbose=0):
        if not(os.access(path, os.F_OK)):
            raise DICOMReadError('Path ' + path + ' does not exist' )
        if not(os.access(path, os.R_OK)):
            raise DICOMReadError('Path ' + path + ' does not have read permission' )
