...
#
# Apply rigid co-registration with FSL's FNIRT
#
# input_file     - file that is co-registered (.nii.gz)
# reference_file - file where input is moved (.nii.gz)
# out_prefix     - subject specific prefix
#
def FSL_FNIRT_write(input_file, reference_file, matrix_file, output_prefix):
    from nipype.interfaces import fsl
    import os

    flt = fsl.FNIRT(bins=640, cost_func='mutualinfo')
    flt.inputs.in_file =input_file
    flt.inputs.reference = reference_file
    flt.inputs.in_matrix_file = matrix_file
    flt.inputs.apply_xfm = True
    print "FNIRT [" + os.path.basename(input_file) + "->" + os.path.basename(reference_file) + "]:" + flt.cmdline
    res = flt.run()
    outfile = move_to_results(res.outputs.out_file, output_prefix)
    return outfile

def FSL_ConvertXFM_concat(matrix_file, matrix_file):
    from nipype.interfaces import fsl
    import os
    ...
    invt.cmdline    flt = fsl.FNIRT(bins=640, cost_func='mutualinfo')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dicomT1dir", dest="dicomT1dir", help="dicom T1 (anatomical) dir", required=True)
    parser.add_argument("--subject", dest="subject", help="subject id", required=True)
    args = parser.parse_args()

    dwifile = experiment_dir+'/'+args.subject+'/'+args.subject+'DTI_QCed.nii.gz'
    bval_file = experiment_dir+'/'+args.subject+'/'+args.subject+'DTI_QCed.bval'
    bvec_file = experiment_dir+'/'+args.subject+'/'+args.subject+'DTI_QCed.bvec'
    mask_file = runbet(dwifile, args.subject)

    # Copy data to new directory to help DWIconvert work
    path, prefix, ext  = split_filename(args.dicomT1dir)
    if os.path.isdir((path + os.sep + 'MR_T1W')):
        shutil.rmtree((path + os.sep + 'MR_T1W'))
    shutil.copytree(args.dicomT1dir, (path + os.sep + 'MR_T1W'))
    args.dicomT1dir = (path + os.sep + 'MR_T1W')

    # Pre-set T1
    T1_file, T1_file_bval, T1_file_bvec = conversions.dicom2nii(args.dicomT1dir, args.subject, 'T1', experiment_dir)
    T1_file = conversions.rename_basename_to(T1_file, 'MR_T1w')
    T1_file_mask = runbet(T1_file, args.subject)
    T1_at_MNI_file, T1_to_MNI_matrix = FSL_FLIRT_estim(T1_file, MNI_file, args.subject)

    # Normalize DTI to MNI
    DTI_at_T1_file, DTI_to_T1_matrix = FSL_FLIRT_estim(dwifile, T1_file, args.subject)
    
    T1_mask_file = FSL_FLIRT_write(dwifile, MNI_file, DTI_to_T1_matrix, args.subject)

    # Bring MNI ROIs to individual

    # Bring segmentation of T1 to DTI
